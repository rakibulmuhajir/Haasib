...
<template>
  ...
  <Dialog
    ...
    position="bottom"
    :blockScroll="true"
    class="w-full max-w-6xl border-0 bg-transparent shadow-none"
    :pt="{ mask: { class: 'bg-black/70 backdrop-blur-sm' }, root: { class: 'border-0 shadow-none bg-transparent' }, content: { class: 'border-0 shadow-none bg-transparent' } }"
  >
    <div class="w-full max-w-6xl flex flex-col lg:flex-row gap-4 relative mx-auto px-2 pb-2 sm:pb-4"
         :class="[palette.showResults.value ? 'lg:max-w-6xl' : '', dockSize === 'full' ? 'h-[88vh]' : 'h-[56vh]']">
      <!-- Compact toggle -->
      <button
