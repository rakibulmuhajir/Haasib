...
const form = useForm({
    ...
    reference_number: '',
    notes: props.selectedBill
        ? `Payment for ${props.selectedBill.bill_number}`
        : '',
    payment_account_id: '',
    ...
    allocations: props.selectedBill
        ? [
              {
                  bill_id: props.selectedBill.id,
                  amount_allocated: moneyNumber(props.selectedBill.balance),
              },
          ]
...
const hasDefaultApAccount = computed(() =>
    Boolean(props.defaults?.ap_account_id),
);
...
const fetchUnpaidBills = (vendorId: string) => {
    ...
    router.get(
        ...
        { vendor_id: vendorId },
        {
            ...
            onSuccess: (page) => {
                const bills =
                    ((page.props as any).unpaidBills as BillRef[]) ?? [];
                unpaidBills.value = bills;
                resetAllocations(unpaidBills.value);
            },
        },
...
const totalAllocated = computed(() =>
    form.allocations.reduce(
        (sum, a) => sum + (Number(a.amount_allocated) || 0),
        0,
    ),
);

const totalSplit = computed(() =>
    form.payment_splits.reduce(
        (sum, split) => sum + (Number(split.amount) || 0),
        0,
    ),
);

const selectedBillBalance = computed(() =>
    moneyNumber(props.selectedBill?.balance),
);

const totalTransactionCharge = computed(() =>
    moneyNumber(form.transaction_charge),
);

const totalCashMovement = computed(() =>
    moneyNumber(form.amount + totalTransactionCharge.value),
);
    ...
    props.selectedBill
        ...
        : unpaidBills.value.reduce((sum, bill) => {
              const allocated = Number(
                  form.allocations.find((row) => row.bill_id === bill.id)
                      ?.amount_allocated || 0,
              );
...
const remainingVendorCredit = computed(() =>
    Math.max(
        0,
        Number((allocatedBillBalance.value - totalAllocated.value).toFixed(2)),
    ),
);
...
const apAccountLabel = computed(() => {
    const accountId =
        form.ap_account_id === '__none' || !form.ap_account_id
            ? props.defaults?.ap_account_id
            : form.ap_account_id;
    const account = (props.apAccounts || []).find(
...
const billRemainingAfterAllocation = (bill: BillRef) => {
    ...
    return Math.max(
        0,
        moneyNumber(bill.balance) - moneyNumber(allocation?.amount_allocated),
    );
};
        ...
        <form novalidate class="space-y-6" @submit.prevent="handleSubmit">
            <div class="grid gap-4 md:grid-cols-2">
                <div>
                    ...
                    <Select
                        ...
                        @update:modelValue="fetchUnpaidBills"
                    >
                        <SelectTrigger>
                ...
                <div>
                    <Label for="transaction_charge">Transaction Charges</Label>
                    <Input
                    ...
                    <p class="mt-1 text-xs text-muted-foreground">
                        Bank or transfer fee added to the cash/bank payment.
                    </p>
                </div>
                <div>
                    ...
                    <Select v-model="form.ap_account_id">
                        <SelectTrigger
                            id="ap_account_id"
                            :class="{
                                'border-destructive': form.errors.ap_account_id,
                            }"
                        >
                            <SelectValue
                                :placeholder="
                    ...
                    <p
                        v-if="form.errors.ap_account_id"
            ...
            <div class="space-y-3">
                <div class="flex items-center justify-between gap-3">
                    <div>
                        ...
                        <p class="text-sm text-muted-foreground">
                            Add only the amount you are paying now. Any unpaid
                            bill balance stays as vendor credit in Accounts
                            Payable.
                        </p>
                    </div>
                    <Button
                ...
                <div class="space-y-3">
                    ...
                    >
                        <div>
                            ...
                            <p
                                v-if="accountById(split.payment_account_id)"
                                class="mt-1 text-xs text-muted-foreground"
                            >
                                Est. balance
                                {{
                                    formatNumber(
                                        accountById(split.payment_account_id)
                                            ?.estimated_balance ?? 0,
                                    )
                                }}
                                · After payment
                                {{ formatNumber(projectedBalance(split) ?? 0) }}
                            </p>
                            <InputError
                                :message="
                                    form.errors[
                                        `payment_splits.${index}.payment_account_id`
                                    ]
                                "
                            />
                        </div>
                        ...
                        <div>
                            ...
                            <InputError
                                :message="
                                    form.errors[
                                        `payment_splits.${index}.payment_method`
                                    ]
                                "
                            />
                        </div>
                        ...
                        <div>
                            ...
                            <InputError
                                :message="
                                    form.errors[
                                        `payment_splits.${index}.reference_number`
                                    ]
                                "
                            />
                        </div>
                ...
                <div class="flex justify-between text-sm">
                    ...
                    <MoneyText
                        :amount="totalTransactionCharge"
                        :currency="form.currency"
                    />
                </div>
                <div class="flex justify-between font-semibold">
                    <span>Total cash/bank movement</span>
                    <MoneyText
                        :amount="totalCashMovement"
                        :currency="form.currency"
                    />
                </div>
                <div
                    ...
                    >
                        <div>
                            ...
                            <div class="text-xs text-muted-foreground">
                                Balance {{ formatNumber(bill.balance) }}
                                {{ bill.currency }}
                            </div>
                            <div class="text-xs text-muted-foreground">
                                ...
                                {{
                                    formatNumber(
                                        billRemainingAfterAllocation(bill),
                                    )
                                }}
                                {{ bill.currency }}
                            </div>
                        ...
                        <Input
                            ...
                            step="0.01"
                            v-model.number="
                                getAllocation(bill.id).amount_allocated
                            "
            ...
            <div
                class="grid gap-3 rounded-sm border bg-muted/30 p-4 text-sm md:grid-cols-3"
            >
                <div>
                ...
                <div>
                    <div class="text-muted-foreground">
                        Remaining vendor credit
                    </div>
                    <div class="text-lg font-semibold">
                        {{ formatNumber(remainingVendorCredit) }}
                        {{ form.currency }}
                    </div>
                </div>
                <div>
                    ...
                    <p class="mt-1 text-xs text-muted-foreground">
                        The remaining amount is already carried here from the
                        bill posting.
                    </p>
                </div>
            </div>
