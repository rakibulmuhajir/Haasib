...
const breadcrumbs: BreadcrumbItem[] = [
    ...
    {
        title: props.editingVoucher ? 'Edit Voucher' : 'New Voucher',
        href: props.editingVoucher
            ? `/${props.company.slug}/umrah/vouchers/${props.editingVoucher.id}/edit`
            : `/${props.company.slug}/umrah/vouchers/create`,
    },
];
const page = usePage();
...
const form = useForm({
    ...
    onward_departure_city: props.editingVoucher?.onward_departure_city || '',
    onward_arrival_city: props.editingVoucher?.onward_arrival_city || '',
    onward_departure_at: localDateTime(
        props.editingVoucher?.onward_departure_at,
    ),
    onward_arrival_at: localDateTime(props.editingVoucher?.onward_arrival_at),
    return_airline: props.editingVoucher?.return_airline || '',
    return_flight_number: props.editingVoucher?.return_flight_number || '',
    return_departure_city: props.editingVoucher?.return_departure_city || '',
    return_arrival_city: props.editingVoucher?.return_arrival_city || '',
    return_departure_at: localDateTime(
        props.editingVoucher?.return_departure_at,
    ),
    return_arrival_at: localDateTime(props.editingVoucher?.return_arrival_at),
    hotel_stays: editableStays?.length
        ? editableStays
        : [
              {
                  source: 'self',
...
const stayWindowStart = computed(() =>
    hotelOnly.value
        ? undefined
        : localDate(form.onward_arrival_at) || undefined,
);
const stayWindowEnd = computed(() =>
    hotelOnly.value
        ? undefined
        : localDate(form.return_departure_at) || undefined,
);
const airlineOptions = computed(() =>
    Object.entries(props.airlines).map(([value, label]) => ({ value, label })),
);
const cityOptions = computed(() =>
    Object.entries(props.airportCities).map(([value, label]) => ({
        value,
        label,
    })),
);
const hotelOptionsFor = (city: string) =>
    props.hotels
...
watch(
    ...
    (passengers) => {
        selectedPassengerIds.value = passengers.map(
            (passenger) => passenger.id,
        );
        passengerServices.value = Object.fromEntries(
...
watch(
    () => form.onward_airline,
    (value, previous) => {
        if (!form.return_airline || form.return_airline === previous)
            form.return_airline = value;
    },
);

watch(
    () => form.onward_departure_city,
    (value, previous) => {
        if (!form.return_arrival_city || form.return_arrival_city === previous)
            form.return_arrival_city = value;
    },
);

watch(
    () => form.onward_arrival_city,
    (value, previous) => {
        if (
            !form.return_departure_city ||
            form.return_departure_city === previous
        )
            form.return_departure_city = value;
    },
);
...
const changeGroup = (groupId: string) => {
    ...
    router.get(`/${props.company.slug}/umrah/vouchers/create`, params, {
        preserveState: false,
    });
};
...
const selectHotel = (index: number, hotelId: string) => {
    ...
    if (
        !hotel.room_rates.some((rate: any) => rate.room_type === stay.room_type)
    )
        stay.room_type = hotel.room_rates[0]?.room_type || 'none';
};
...
const selectedHotel = (hotelId: string) =>
    props.hotels.find((hotel) => hotel.id === hotelId);
const bedsPerRoom: Record<string, number> = {
    sharing: 1,
    double: 2,
    triple: 3,
    quad: 4,
...
const stayAmount = (stay: any, field: 'retail_amount' | 'cost_amount') => {
    ...
    const rate = selectedHotel(stay.hotel_id)?.room_rates?.find(
        (item: any) => item.room_type === stay.room_type,
    );
    if (!rate || !stay.check_in_date || !stay.check_out_date) return 0;
    const nights = Math.max(
        Math.round(
            (new Date(stay.check_out_date).setHours(0, 0, 0, 0) -
                new Date(stay.check_in_date).setHours(0, 0, 0, 0)) /
                86_400_000,
        ),
        1,
    );
    return (
        Number(rate[field] || 0) *
        (bedsPerRoom[stay.room_type] || 1) *
        Number(stay.room_count || 1) *
        nights
    );
};
const hotelSale = computed(() =>
    form.hotel_stays.reduce(
        (total, stay) => total + stayAmount(stay, 'retail_amount'),
        0,
    ),
);
const hotelCost = computed(() =>
    form.hotel_stays.reduce(
        (total, stay) => total + stayAmount(stay, 'cost_amount'),
        0,
    ),
);
const includesHotelService = computed(
    () =>
        hotelOnly.value ||
        form.hotel_stays.some((stay) => stay.source === 'company'),
);
const voucherServiceLabel = computed(() =>
    hotelOnly.value ? 'Hotel only' : 'Passenger services selected below',
);
...
const submit = () => {
    form.transform((data) => ({
        ...
        hotel_stays: data.hotel_stays.map((stay) => ({
            ...stay,
            hotel_id: stay.hotel_id === 'none' ? null : stay.hotel_id,
            room_type: stay.room_type === 'none' ? null : stay.room_type,
            room_count: Number(stay.room_count || 1),
        })),
    }));
    ...
    if (editingVoucher.value) {
        form.put(
            `/${props.company.slug}/umrah/vouchers/${editingVoucher.value.id}`,
            options,
        );
        return;
    }
...
<template>
    ...
    <PageShell
        ...
        :description="
            editingVoucher
                ? 'Update the draft journey schedule and stays.'
                : 'Create a journey schedule for all or selected group members.'
        "
            ...
            <div class="space-y-6">
                <Card>
                    <CardHeader>
                        ...
                        <CardDescription>{{
                            editingVoucher
                                ? 'The group and passengers are fixed after voucher creation.'
                                : 'Pick the group first. Remaining members appear below.'
                        }}</CardDescription>
                    </CardHeader>
                    <CardContent class="space-y-4">
                        <div class="grid gap-4 md:grid-cols-2">
                            <div class="space-y-2">
                                <Label>Group</Label>
                                <Select
                                    v-model="form.visa_group_id"
                                    :disabled="!!editingVoucher"
                                    @update:model-value="
                                        (value) => changeGroup(String(value))
                                    "
                                >
                                    <SelectTrigger
                                        ><SelectValue
                            ...
                            <div
                                class="flex items-start gap-3 rounded-md border p-3 md:col-span-2"
                            >
                                <Checkbox id="hotel-only" v-model="hotelOnly" />
                                <div>
                                    <Label for="hotel-only"
                                        >Hotel-only voucher</Label
                                    >
                                    <p class="text-sm text-muted-foreground">
                                        No flight or transport schedule is
                                        required.
                                    </p>
                                </div>
                                <p
                                    v-if="form.errors.service_bundle"
                                    class="text-xs text-destructive"
                                >
                                    {{ form.errors.service_bundle }}
                                </p>
                            </div>
                        </div>
                    </CardContent>
                ...
                <Card v-if="!editingVoucher">
                    ...
                    <CardContent class="space-y-3">
                        ...
                        >
                            Select a group to choose passengers.
                        </div>
                        <div
                            v-else-if="!availablePassengers.length"
                            class="rounded-md border p-4 text-sm text-muted-foreground"
                        >
                            All current group members already have vouchers.
                        </div>
                        <template v-else>
                            <div
                            ...
                            <div
                                v-for="passenger in availablePassengers"
                                :key="passenger.id"
                            ...
                            >
                                ...
                                >
                                    <Checkbox
                                        :id="`transport-only-${passenger.id}`"
                                        :model-value="
                                            passengerServices[passenger.id] ===
                                            'transport_only'
                                        "
                                        @update:model-value="
                                            (checked) =>
                                    ...
                                    />
                                    <Label
                                        :for="`transport-only-${passenger.id}`"
                ...
                <Card v-if="!hotelOnly">
                    <CardHeader>
                        ...
                        <CardDescription
                            >Hotel stay dates must fit between these flight
                            dates.</CardDescription
                        >
                    </CardHeader>
                    <CardContent class="space-y-3">
                        <div
                            class="hidden gap-3 px-3 text-xs text-muted-foreground 2xl:grid 2xl:grid-cols-[72px_90px_95px_100px_100px_minmax(160px,1fr)_minmax(160px,1fr)]"
                        >
                            <span>Sector</span><span>Airline</span
                            ><span>Flight #</span><span>From</span
                            ><span>To</span><span>Takeoff</span
                            ><span>Landing</span>
                        </div>
                        <div
                            class="grid min-w-0 gap-3 rounded-md border p-3 md:grid-cols-2 2xl:grid-cols-[72px_90px_95px_100px_100px_minmax(160px,1fr)_minmax(160px,1fr)] 2xl:items-end"
                        >
                            <div class="font-medium 2xl:pb-2">Onward</div>
                            <div class="min-w-0 space-y-1">
                                <Label class="2xl:hidden">Airline</Label
                                ><SearchableSelect
                            ...
                            <div class="min-w-0 space-y-1">
                                <Label class="2xl:hidden">Flight #</Label
                                ><Input
                                    v-model="form.onward_flight_number"
                                    maxlength="5"
                                    placeholder="PK741"
                                />
                            </div>
                            <div class="min-w-0 space-y-1">
                                <Label class="2xl:hidden">From</Label
                                ><SearchableSelect
                            ...
                            <div class="min-w-0 space-y-1">
                                <Label class="2xl:hidden">To</Label
                                ><SearchableSelect
                        ...
                        >
                            ...
                            <div class="min-w-0 space-y-1">
                                <Label class="2xl:hidden">Flight #</Label
                                ><Input
                                    v-model="form.return_flight_number"
                                    maxlength="5"
                                    placeholder="PK742"
                                />
                            </div>
                ...
                <Card>
                    <CardHeader>
                        ...
                        <CardDescription>{{
                            includesHotelService
                                ? 'Stays are included and charged by the company.'
                                : 'Stays complete the itinerary and are not charged on this voucher.'
                        }}</CardDescription>
                    </CardHeader>
                    <CardContent class="space-y-3">
                        <div
                            v-for="(stay, index) in form.hotel_stays"
                            :key="index"
                            class="grid min-w-0 gap-3 rounded-md border p-3 md:grid-cols-2 lg:grid-cols-12 lg:items-end"
                        >
                            <div class="font-medium lg:col-span-1 lg:pb-2">
                                {{ index + 1 }}
                            </div>
                            <div class="min-w-0 space-y-1 lg:col-span-2">
                                <Label>City</Label
                                ><Select
                                    :model-value="stay.city"
                                    @update:model-value="
                                        setStayCity(index, String($event))
                                    "
                                    ><SelectTrigger
                                        ><SelectValue
                                            placeholder="Select city" /></SelectTrigger
                                    ><SelectContent
                            ...
                            <div
                                v-if="stay.source === 'company'"
                                class="min-w-0 space-y-1 lg:col-span-3"
                            >
                                <Label>Hotel</Label
                                ><SearchableSelect
                                    ...
                                    @update:model-value="
                                        selectHotel(index, String($event))
                                    "
                                />
                            </div>
                            <div v-else class="min-w-0 space-y-1 lg:col-span-3">
                                <Label>Hotel</Label
                                ><Input v-model="stay.hotel_name" required />
                            </div>
                            <div class="min-w-0 space-y-1 lg:col-span-2">
                                <Label>Room</Label
                                ><Select v-model="stay.room_type"
                                    ><SelectTrigger
                                        ><SelectValue
                                            placeholder="Type" /></SelectTrigger
                                    ><SelectContent
                                        ><SelectItem
                                            v-for="rate in stay.source ===
                                            'company'
                                                ? selectedHotel(stay.hotel_id)
                                                      ?.room_rates || []
                                                : [
                                                      { room_type: 'sharing' },
                                                      { room_type: 'double' },
                            ...
                            <div class="min-w-0 space-y-1 lg:col-span-1">
                                <Label>{{
                                    stay.room_type === 'sharing'
                                        ? 'Beds'
                                        : 'Rooms'
                                }}</Label
                                ><Input
                                    v-model="stay.room_count"
                                    type="number"
                                    min="1"
                                    required
                                />
                            ...
                            <div
                                class="min-w-0 space-y-1 lg:col-span-3 lg:col-start-2"
                            >
                            ...
                            >
                                <Button
                                    ...
                                    variant="ghost"
                                    size="icon"
                                    :disabled="form.hotel_stays.length === 1"
                                    @click="removeHotelStay(index)"
                                >
                            ...
                            <p
                                v-if="
                                    form.errors[
                                        `hotel_stays.${index}.hotel_name`
                                    ]
                                "
                                class="text-xs text-destructive lg:col-span-12"
                            >
                                {{
                                    form.errors[
                                        `hotel_stays.${index}.hotel_name`
                            ...
                            <p
                                v-if="
                                    form.errors[
                                        `hotel_stays.${index}.check_in_date`
                                    ]
                                "
                                class="text-xs text-destructive lg:col-span-12"
                            >
                            ...
                            <p
                                v-if="
                                    form.errors[
                                        `hotel_stays.${index}.check_out_date`
                                    ]
                                "
                                class="text-xs text-destructive lg:col-span-12"
                            >
            ...
            <div class="space-y-6">
                <Card>
                    <CardHeader>
                        ...
                        <CardDescription
                            >Save as draft or approve the completed
                    ...
                    <CardContent class="space-y-4">
                        <div class="rounded-md border p-3 text-sm">
                            ...
                            <div class="text-muted-foreground">
                                {{ selectedGroup?.agent?.name || 'No agent' }}
                            </div>
                            <div class="mt-2">
                                {{
                                    editingVoucher
                                        ? editingVoucher.passengers?.length || 0
                                        : selectedPassengerIds.length
                                }}
                                passengers
                            </div>
                            ...
                            <div
                                v-if="includesHotelService && canViewAccounting"
                        ...
                        <div
                            v-if="canApprove && !editingVoucher"
                            class="space-y-2"
                        >
                            <Label>Status</Label>
                        ...
                        <div v-else class="rounded-md border p-3 text-sm">
                ...
                <Card v-if="assignedPassengers.length">
                    <CardHeader>
                        ...
                        <CardDescription
                            >These members will not appear in the available
                            list.</CardDescription
                        >
                    </CardHeader>
                    <CardContent class="space-y-3">
                        <div
                            v-for="passenger in assignedPassengers"
                            :key="passenger.id"
