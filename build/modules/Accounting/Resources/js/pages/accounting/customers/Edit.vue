...
import { Pencil, ArrowLeft, Save } from 'lucide-vue-next'
...
const form = useForm({
  ...
  base_currency: props.customer.base_currency,
  ...
  billing_address: {
    street: props.customer.billing_address?.street || '',
    city: props.customer.billing_address?.city || '',
    state: props.customer.billing_address?.state || '',
    zip: props.customer.billing_address?.zip || '',
    country: props.customer.billing_address?.country || '',
  },
  shipping_address: {
    street: props.customer.shipping_address?.street || '',
    city: props.customer.shipping_address?.city || '',
    state: props.customer.shipping_address?.state || '',
    zip: props.customer.shipping_address?.zip || '',
    country: props.customer.shipping_address?.country || '',
  },
})

const handleSubmit = () => {
  form.put(`/${props.company.slug}/customers/${props.customer.id}`)
}

const handleCancel = () => {
  router.visit(`/${props.company.slug}/customers/${props.customer.id}`)
}
</script>

<template>
  <Head :title="`Edit ${customer.name} - ${company.name}`" />
  <PageShell
    title="Edit Customer"
    :icon="Pencil"
  ...
  >
    <template #description>
      Editing <span class="font-medium text-slate-300">{{ customer.name }}</span>
      <span class="text-slate-500 ml-2">({{ customer.customer_number }})</span>
    </template>
        ...
        <CardContent class="space-y-4">
          ...
          <div class="flex items-center justify-between pt-4 border-t border-slate-800">
            <div>
              ...
              <p class="text-xs text-slate-500">Inactive customers won't appear in lists by default</p>
            </div>
