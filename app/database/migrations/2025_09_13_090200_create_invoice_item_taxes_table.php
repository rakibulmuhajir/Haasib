...
return new class extends Migration
{
    ...
    public function up(): void
    {
        ...
        Schema::table('acct.invoice_item_taxes', function (Blueprint $table) {
            $table->foreign('invoice_item_id')->references('invoice_item_id')->on('acct.invoice_items')->onDelete('cascade');
        ...
        try {
            DB::statement('CREATE UNIQUE INDEX IF NOT EXISTS invoice_item_taxes_idemp_unique ON acct.invoice_item_taxes (invoice_item_id, idempotency_key) WHERE idempotency_key IS NOT NULL');
        ...
        DB::statement(<<<'SQL'
            CREATE POLICY invoice_item_taxes_tenant_isolation ON acct.invoice_item_taxes
            USING (EXISTS (
                SELECT 1 FROM acct.invoice_items ii
                JOIN acct.invoices i ON i.invoice_id = ii.invoice_id
