...
interface CurrencyManagementTableProps {
  currencies: Currency[]
  loading?: boolean
  togglingStates?: Record<string, boolean>
}
...
const toggleActive = (currency: Currency, nextActive?: boolean) => {
  if (currency.is_base_currency) return // Cannot deactivate base currency
  const target = typeof nextActive === 'boolean' ? nextActive : !currency.is_active
  emit('toggle', currency.id, target)
}
          ...
          <TableRow v-else v-for="currency in sortedCurrencies" :key="currency.id">
            ...
            <TableCell>
              <div class="flex items-center gap-2">
                <Switch
                  :model-value="currency.is_active"
                  @update:modelValue="(value) => toggleActive(currency, value)"
                  :disabled="currency.is_base_currency || props.togglingStates[currency.id]"
                />
                <span class="text-sm flex items-center gap-1">
                  {{ currency.is_active ? 'Active' : 'Inactive' }}
                  <span
                    v-if="props.togglingStates[currency.id]"
            ...
            <TableCell class="text-right">
              <div class="flex items-center justify-end gap-2">
                <span v-if="currency.is_base_currency" class="text-sm text-muted-foreground italic">
                  Cannot be deleted
                </span>
                <Button
                  v-else
