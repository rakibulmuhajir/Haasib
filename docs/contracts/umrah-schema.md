...
### umrah.vouchers
- `leader_passenger_id` uuid nullable FK -> `umrah.passengers.id` (on delete set null), fillable and cast to string. Explicitly selected travel-party leader; must be one of this voucher's selected/current passengers, including authorized transferred passengers. At most one leader per voucher. Null means not selected (including legacy records); never infer leadership from row order. Draft edits may change/clear it; approved copies require amendment. Moving the leader out clears the source selection without assigning another person. Amendments preserve it; separated copies retain it only if that passenger belongs to the copy. No accounting effect.
...
- Vouchers represent travelling parties, not visa/import batches. Previously purchased visa and transport remain with the original purchasing agent and source group; later services belong to the agent purchasing them, not automatically to the passenger's original agent.
- Cross-group/cross-agent draft transfers are company-staff actions. Agent logins may move passengers only between their own vouchers. The passenger and assignment retain their original `visa_group_id`; moving membership must not recalculate groups, transfer payments, or post/reverse any charges.
- Group detail shows original purchase passengers with their current active travelling voucher, plus a separate list of passengers joining that group's vouchers from other purchase groups. These are read-only projections of live assignments, not changes to group membership/counts or balances. Cancelled, deleted, superseded vouchers and pending amendment drafts are excluded. Agent users receive no foreign voucher/group links, commercial details, or private notes. Visa-processing status controls are not part of the group detail workflow.
- Transfer scope: ordinary drafts and current approved vouchers may exchange passengers. Issued-voucher moves require company staff with update and approval permission plus a reason. Pending amendments and shared-billing copies/owners remain blocked. Approved hotel purchases, journal references and balances are unchanged; no accounting amendment is created. Both change logs retain before/after manifests (identity, original group and leader) and the unchanged itinerary/purchase snapshot. Cross-group moves and issued-voucher moves may empty the source without deleting or cancelling its purchases; empty drafts cannot be approved. Empty approved vouchers retain hotel booking visibility but produce no flight/city movement events. Mixed-source vouchers require valid original purchase groups before receiving passengers while approved.
- Mixed-source print output maps passenger row numbers to original visa/transport providers, with at most four header slots. It contains no original-agent prices or private notes. Transport checks use original purchase groups; hotel Operations entries remain one party booking so rooms are not multiplied. Scheduled transport manifests use passengers from the original purchase group regardless of the current voucher's agent.
- Scheduled pickup passenger suggestions must additionally match the current approved voucher's local arrival/departure date and route, or its hotel city-transfer dates. Never use the entire import batch as a pickup manifest. An explicit scheduled headcount remains visible; disagreement with itinerary-matched passengers is flagged for review. Missing itinerary matches remain needs-attention, not a fabricated passenger list. These are itinerary-derived suggestions, not a persisted per-vehicle passenger allocation.
- Hotel reversal recalculates supplier balances excluding the reversed voucher before the lifecycle transaction applies its cancelled/superseded marker; amendments must not temporarily count both old and replacement stays in supplier totals.
...
- Approved voucher commercial fields are immutable outside the amendment/reversal workflow. The audited travelling-party transfer is a non-financial exception; it must never reprice, reverse or transfer existing purchases.
- Issued-party transfers increment the current approved voucher's `version_number` for reprinting. Before/after manifest versions and the unchanged purchase snapshot are retained in `change_logs.metadata`; this does not create a financial amendment or supersede the purchase record. Agents receive no change-log payload; staff lacking financial visibility do not receive the commercial snapshot.
...
- Operations movements are read-only projections, not independently editable records: airport arrival uses voucher `onward_arrival_at`, Saudi departure uses `return_departure_at`, hotel check-in/out use each stay's dates, and road movements use transport item `scheduled_at`. A consecutive Makkah/Madinah stay change without a matching transport item produces a date-only city-transfer warning from the departing stay's `check_out_date`; the precise transport event replaces that fallback once scheduled.
- Operations includes only approved, non-cancelled, non-superseded vouchers and non-cancelled groups. Active voucher passenger pivots are the manifest authority.
- Travel datetimes are local wall-clock values. Operations preserves each stored clock value and calculates Today/Tonight/Tomorrow using the movement location's local date; viewer/browser timezone never converts the displayed time.
- Event payload detail is role-shaped on the server. Owner, operations, and manager profiles receive company operational detail; accountant summary profiles receive aggregate movement counts only; agent profiles receive detail only after linked-agent scoping. The owner's Umrah dashboard carries a separate summary-only Operations tab so full access does not make the operational register the owner's landing page.
