...
class CreateAction implements PaletteAction
{
    public function rules(): array
    {
        return [
            ...
            'status' => 'nullable|string|in:draft,issued',
    ...
    {
        return Permissions::CREDIT_NOTE_CREATE;
    }
    ...
    public function handle(array $params): array
    {
        ...
        app(DocumentDateLock::class)->assertOpen($company->id, $creditDate->toDateString(), 'This credit note');
        ...
        return DB::transaction(function () use ($company, $customer, $invoice, $creditDate, $status, $amount, $baseCurrency, $params, $lineItems) {
            $creditNumber = CreditNote::generateCreditNoteNumber($company->id);
            ...
            return [
                'message' => "Credit note {$creditNumber} created ({$status})",
                'data' => [
                    'id' => $credit->id,
                    'number' => $creditNumber,
                    'amount' => $amount,
