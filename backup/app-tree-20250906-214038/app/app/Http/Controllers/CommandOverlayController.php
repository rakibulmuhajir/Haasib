    ...
    public function index(Request $request)
    {
        ...
        $rows = DB::table('command_overlays')
            ->where(function ($q) use ($companyId, $user) {
                ...
                if ($companyId) {
                    $q->orWhere(function ($q2) use ($companyId) { $q2->where('company_id', $companyId)->whereNull('user_id'); });
                }
                $q->orWhere(function ($q3) use ($user) { $q3->whereNull('company_id')->where('user_id', $user->id); });
                if ($companyId) {
                    $q->orWhere(function ($q4) use ($companyId, $user) { $q4->where('company_id', $companyId)->where('user_id', $user->id); });
                }
            })
