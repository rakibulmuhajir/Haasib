...
class UpdateCustomerAddressAction
{
     ...
     */
    public function execute(CustomerAddress $address, CustomerAddressData $data): CustomerAddress
    {
        // Validate data
        $this->validate($data);

        return DB::transaction(function () use ($address, $data) {
            // Handle default address logic if changing type or setting as default
            $wasDefault = $address->is_default;
            $typeChanged = $address->type !== $data->type;
            $settingAsDefault = $data->is_default && ! $wasDefault;

            if ($settingAsDefault || ($typeChanged && $data->is_default)) {
                // Unset existing default address for the type
            ...
            // Emit audit event
            $this->emitAuditEvent('customer_address_updated', $address, $oldValues);
    ...
    {
        // Validate country-specific requirements (same as create action)
     ...
     */
    private function emitAuditEvent(string $event, CustomerAddress $address, array $oldValues): void
    {
