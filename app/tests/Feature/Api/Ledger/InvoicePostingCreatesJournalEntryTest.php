...
it('creates a journal entry when posting an invoice (via API)', function () {
    ...
    $invoiceData = $this->withHeaders($headers)->postJson('/api/invoices', $createPayload)
        ->assertStatus(201)->json('data');

    $invoiceId = $invoiceData['invoice_id'];

    // Send then post invoice to ledger
    $postHeaders = [
        'X-Company-Id' => $company->id,
        'Idempotency-Key' => (string) Str::uuid(),
    ];
    $this->withHeaders($postHeaders)->postJson("/api/invoices/{$invoiceId}/send")->assertStatus(200);
    $this->withHeaders($postHeaders)->postJson("/api/invoices/{$invoiceId}/post")
        ->assertStatus(200);
