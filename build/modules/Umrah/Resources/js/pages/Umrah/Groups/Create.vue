...
import { Plane, Plus, Save, Trash2, Upload } from 'lucide-vue-next';
...
watch(
    ...
    (value, oldValue) => {
        if (sameAmount(vendorForm.child_retail_amount, oldValue))
            vendorForm.child_retail_amount = value;
    },
);
...
watch(
    ...
    (value, oldValue) => {
        if (sameAmount(vendorForm.child_cost_amount, oldValue))
            vendorForm.child_cost_amount = value;
    },
);

const receivable = computed(() => {
    return Math.max(
        Number(form.visa_sale_amount || 0) +
            Number(form.transport_amount || 0) -
...
const profit = computed(
    () =>
        receivable.value -
        Number(form.visa_cost_amount || 0) -
        Number(form.transport_cost_amount || 0),
);
const selectedAgent = computed(() =>
    props.agents.find((item) => item.id === form.agent_id),
);
...
const transportFareTotals = computed(() =>
    form.transport_items.reduce(
        (totals, item) => {
            ...
            const pax = Math.max(
                Number(
                    item.passenger_count || namedPassengers.value.length || 1,
                ),
                1,
            );
            const factor =
            ...
            totals.sale +=
                (Number(fare.sale_amount || 0) +
                    (hajj ? Number(fare.hajj_terminal_sale_amount || 0) : 0)) *
                factor;
            totals.cost +=
                (Number(fare.cost_amount || 0) +
                    (hajj ? Number(fare.hajj_terminal_cost_amount || 0) : 0)) *
                factor;
            return totals;
        },
        { sale: 0, cost: 0 },
...
const ageBand = (passenger: {
    date_of_birth?: string | null;
    imported_age?: string | number | null;
}) => {
    const importedAge =
    ...
    if (!normalizedBirthDate)
        return importedAge !== null && importedAge < 12 ? 'child' : 'adult';

    const birthDate = new Date(`${normalizedBirthDate}T00:00:00`);
    const normalizedTravelDate = normalizeDate(form.travel_date);
    const referenceDate = normalizedTravelDate
        ? new Date(`${normalizedTravelDate}T00:00:00`)
        : new Date();
...
const calculateVisaPricing = () => {
    ...
    return passengersForPricing.reduce(
        (totals, passenger) => {
            const band = ageBand(passenger);
            return {
                sale:
                    totals.sale + Number(vendor[`${band}_retail_amount`] || 0),
    ...
    () => {
        ...
        form.passengers.forEach((passenger) => {
            if (
                ...
                passenger.nationality === 'Pakistan'
            ) {
...
const addTransportItem = () => {
    form.transport_items.push({
        ...
        terminal: 'standard',
        quantity: '1',
        passenger_count: String(namedPassengers.value.length || 1),
        notes: '',
    });
};
...
const importedMutamersFlash = computed(
    () => ((page.props.flash as any)?.umrahImportedMutamers || []) as any[],
);
const groupFormErrors = computed(
    () => Object.entries(form.errors) as Array<[string, string]>,
);
...
const removePassenger = (index: number) => {
    form.passengers = form.passengers.filter(
    ...
    const passengerErrors = Object.keys(form.errors).filter(
        (field) => field === 'passengers' || field.startsWith('passengers.'),
    );
...
const submit = () => {
    form.transform((data) => ({
        ...
        transport_service_id:
            data.transport_service_id === 'none'
                ? null
        ...
        transport_items:
            data.transport_mode === 'specialized'
                ? data.transport_items.map((item) => ({
                      ...item,
                      driver_id:
                          item.driver_id === 'none' ? null : item.driver_id,
        ...
        <form class="space-y-6" @submit.prevent="submit">
            ...
            >
                ...
                <ul class="mt-2 list-disc space-y-1 pl-5">
                    <li v-for="[field, error] in groupFormErrors" :key="field">
                        {{ error }}
                    </li>
                </ul>
            </div>
                        ...
                        <CardContent class="grid gap-4 md:grid-cols-2">
                            <div class="space-y-2">
                                ...
                                <Input
                                    ...
                                    :placeholder="`Auto: ${nextGroupNumber}`"
                                />
                                <p
                                    v-if="form.errors.group_number"
                                    class="text-xs text-destructive"
                                >
                                    {{ form.errors.group_number }}
                                </p>
                            </div>
                            <div class="space-y-2">
                                ...
                                <Input
                                    v-model="form.name"
                                    placeholder="Auto: agent name, pax, date and time"
                                />
                                <p
                            ...
                            <div
                                v-if="quickAgentOpen && canManageSetup"
                                class="space-y-3 rounded-md border p-3 md:col-span-2"
                            >
                                <div class="grid gap-3 md:grid-cols-2">
                                    <div class="space-y-2">
                                        ...
                                        />
                                        <p
                                            v-if="agentForm.errors.name"
                                            class="text-xs text-destructive"
                                        >
                                            {{ agentForm.errors.name }}
                                        </p>
                                    </div>
                                    <div class="space-y-2">
                                        <Label>Phone</Label>
                            ...
                            <div class="space-y-2">
                                ...
                                <Select v-model="form.vendor_id">
                                    <SelectTrigger
                                        ><SelectValue
                                            placeholder="Select visa vendor"
                                    /></SelectTrigger>
                                    <SelectContent>
                                ...
                                >
                                    ...
                                    {{
                                        Number(
                                            selectedVendor.adult_retail_amount ||
                                                0,
                                        ).toLocaleString()
                                    }}
                                    /
                                    {{
                                        Number(
                                            selectedVendor.adult_cost_amount ||
                                                0,
                                        ).toLocaleString()
                                    }}
                                    · Child
                                    {{
                                        Number(
                                            selectedVendor.child_retail_amount ||
                                                0,
                                        ).toLocaleString()
                                    }}
                                    /
                                    {{
                                        Number(
                                            selectedVendor.child_cost_amount ||
                                                0,
                                        ).toLocaleString()
                                    }}
                                </div>
                            ...
                            <div
                                v-if="quickVendorOpen && canManageSetup"
                                class="space-y-3 rounded-md border p-3 md:col-span-2"
                            >
                                <div class="grid gap-3 md:grid-cols-2">
                                    <div class="space-y-2">
                                        ...
                                        <Input
                                            ...
                                            placeholder="Visa provider name"
                                        />
                                        <p
                                        ...
                                        >
                                            ...
                                            <SelectContent>
                                                <SelectItem value="government"
                                                    >Government</SelectItem
                                                >
                                                <SelectItem
                                                    value="visa_provider"
                                                    >Visa Provider</SelectItem
                                                >
                        ...
                        <CardContent class="space-y-4">
                            ...
                            >
                                ...
                                <div class="space-y-2">
                                    ...
                                    <p
                                        v-if="form.errors.mandatory_transport_vendor_id"
                                        class="text-xs text-destructive"
                                    >
                            ...
                            <div v-else class="space-y-3">
                                ...
                                >
                                    No specialized fares are configured. Add
                                    vehicle sector or complete-journey fares in
                                    Transport Services first.
                                </div>
                                <div
                                    v-for="(
                                        item, index
                                    ) in form.transport_items"
                                    :key="index"
                                    class="grid gap-3 rounded-md border p-3 lg:grid-cols-[minmax(220px,1fr)_130px_130px_160px_170px_40px]"
                                >
                                    <div class="space-y-1">
                                        <Label
                                        ...
                                        >
                                        <Select
                                            v-model="item.transport_fare_id"
                                        >
                                            <SelectTrigger
                                                ><SelectValue
                                                    placeholder="Select sector or journey"
                                            /></SelectTrigger>
                                            <SelectContent>
                                                <SelectItem
                                        ...
                                        <p
                                            v-if="
                                                nestedError(
                                                    `transport_items.${index}.transport_fare_id`,
                                                )
                                            "
                                            class="text-xs text-destructive"
                                        >
                                    ...
                                    <div class="space-y-1">
                                        ...
                                        <p
                                            v-if="
                                                nestedError(
                                                    `transport_items.${index}.quantity`,
                                                )
                                            "
                                            class="text-xs text-destructive"
                                        >
                                    ...
                                    <div class="space-y-1">
                                        ...
                                        <p
                                            v-if="
                                                nestedError(
                                                    `transport_items.${index}.passenger_count`,
                                                )
                                            "
                                            class="text-xs text-destructive"
                                        >
                                    ...
                                    <div class="space-y-1">
                                        ...
                                        <p
                                            v-if="
                                                nestedError(
                                                    `transport_items.${index}.terminal`,
                                                )
                                            "
                                            class="text-xs text-destructive"
                                        >
                                    ...
                                    <div class="space-y-1">
                                        ...
                                        <p
                                            v-if="
                                                nestedError(
                                                    `transport_items.${index}.scheduled_at`,
                                                )
                                            "
                                            class="text-xs text-destructive"
                                        >
                                    ...
                                    >
                                        {{
                                            fareFor(item.transport_fare_id)
                                                ?.sector?.name ||
                                            fareFor(item.transport_fare_id)
                                                ?.package?.name
                                        }}
                                        ...
                                        {{
                                            ...
                                            )?.charging_basis?.replaceAll(
                                                '_',
                                                ' ',
                                            )
                                        }}
                                        <span v-if="item.terminal === 'hajj'">
                                            · Hajj Terminal surcharge
                                            applied</span
                                        >
                                    </div>
                                </div>
                                <Button
                                    ...
                                    ><Plus class="mr-2 h-4 w-4" />Add Journey or
                                    Sector</Button
                                >
                                <p
                                ...
                                <div class="rounded-md border p-3 text-sm">
                                    ...
                                    >
                                        The selected specialized fare cost is
                                        added below as transport cost.
                                    </div>
                                </div>
                            </div>
                        </CardContent>
                            ...
                            <div class="rounded-md border p-3">
                                ...
                                >
                                    <div class="space-y-2">
                                        ...
                                        <p
                                            v-if="
                                                importForm.errors.mutamers_file
                                            "
                                            class="text-xs text-destructive"
                                        >
                                            {{
                            ...
                            >
                                ...
                                <div class="space-y-1">
                                    ...
                                    <p
                                        v-if="
                                            nestedError(
                                                `passengers.${index}.passport_number`,
                                            )
                                        "
                                        class="text-xs text-destructive"
                                    >
                                ...
                                <div class="space-y-1">
                                    ...
                                    >
                                    <Input
                                        v-model="passenger.imported_age"
                                        type="number"
                                        min="0"
                                        max="130"
                                    />
                                ...
                                <div class="flex items-end">
                                    <Button
                                        ...
                                        variant="ghost"
                                        size="icon"
                                        :aria-label="`Delete ${passenger.full_name || `passenger ${index + 1}`}`"
                                        title="Delete passenger"
                                        @click.stop="removePassenger(index)"
                                        ><Trash2 class="h-4 w-4"
                                    /></Button>
                                </div>
                            </div>
                    ...
                    <Card>
                        ...
                        <CardContent class="space-y-4">
                            ...
                            <div v-if="canViewAccounting" class="space-y-2">
                                ...
                                ><Input
                                    v-model="form.transport_cost_amount"
                                    type="number"
                                    disabled
                                />
                            </div>
