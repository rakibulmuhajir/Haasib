...
class StoreTransportFareRequest extends UmrahFormRequest
{
    ...
    public function rules(): array
    {
        ...
        return [
            ...
            'transport_vendor_id' => ['required', 'uuid', Rule::exists(VisaVendor::class, 'id')->where(fn ($query) => $query->where('company_id', $companyId)->where('service_type', VisaVendor::SERVICE_TRANSPORT_PROVIDER)->where('is_active', true)->whereNull('deleted_at'))],
            'transport_service_id' => ['required', 'uuid', $this->activeForCompany(TransportService::class, 'Select an active vehicle.')],
            'transport_sector_id' => ['nullable', 'uuid', 'required_without:transport_package_id', 'prohibits:transport_package_id', $this->activeForCompany(TransportSector::class, 'Select an active sector.')],
            'transport_package_id' => ['nullable', 'uuid', 'required_without:transport_sector_id', 'prohibits:transport_sector_id', $this->activeForCompany(TransportPackage::class, 'Select an active journey package.')],
