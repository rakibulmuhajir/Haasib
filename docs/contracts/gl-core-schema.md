...
- `fuel.amanat_transactions`, `payroll.salary_advances` and `auth.partner_transactions` created as opening rows carry `reference = 'OPENING'` (informational) and point at their own journal line via `journal_entry_id` (an FK into `acct.journal_entries`, not the transaction) — this is the sole mechanism used to find them on re-save, one join away from `acct.transactions.transaction_type = 'opening_balance'`.
- `company.settings.opening_balances = { as_of_date, locked_at, locked_by_user_id, invoice_ids: [uuid, ...], bill_ids: [uuid, ...] }`. `invoice_ids`/`bill_ids` are the current generation's opening invoice/bill ids — re-saving voids exactly those (via `invoice.void`/`bill.void`, backdated to `as_of_date` through their `reversal_date` param) and replaces both lists with the new generation's ids.
- `as_of_date` must be strictly earlier than the earliest posted non-opening transaction for the company. "Non-opening" excludes: every `transaction_type = 'opening_balance'` row (current or historical — the type never changes across generations); anything already reversed (`reversed_by_id` set) or that is itself a reversal (`reversal_of_id` set); and the current generation's live opening invoice/bill postings (matched by `reference_id` against `invoice_ids`/`bill_ids`). This check (`SaveAction::nonOpeningTransactions()`) is shared by the save guard and by `ViewAction`'s `earliest_transaction_date`.
