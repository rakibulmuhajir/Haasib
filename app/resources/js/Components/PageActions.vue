...
import { usePageActions, resolveDisabled, type PageAction } from '@/composables/usePageActions'
...
const moreBtnMeasure = ref<any>(null)
const visibleCount = ref<number>(props.maxVisible)

function setBtnRef(el: any, idx: number) {
  if (!el) return
  // PrimeVue Button is a component; use its root element
  const node = el.$el ? el.$el as HTMLElement : (el as HTMLElement)
