    ...
    public function rules(): array
    {
        ...
        return [
            ...
            'cash_account_id' => ['required_if:kind,deposit,withdrawal', 'nullable', 'uuid',
                Rule::exists('acct.accounts', 'id')->where(fn ($q) => $q->where('company_id', $companyId)->where('subtype', 'cash')->where('is_active', true))],
            'bank_account_id' => ['required_if:kind,deposit,withdrawal,charge', 'nullable', 'uuid',
                Rule::exists('acct.accounts', 'id')->where(fn ($q) => $q->where('company_id', $companyId)->where('subtype', 'bank')->where('is_active', true))],
