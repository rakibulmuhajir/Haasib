...
return new class extends Migration
{
    public function up(): void
    {
        ...
        collect(config('countries', []))
            ...
            ->unique()
            ->sort()
            ->each(function (string $code) use ($metadata, $now): void {
                $currency = array_merge(
                    ['code' => $code, 'name' => "{$code} Currency", 'symbol' => $code, 'decimal_places' => 2],
                    $metadata[$code] ?? []
                );

                DB::table('public.currencies')->updateOrInsert(
                    ['code' => $currency['code']],
    ...
    {
        // Reference data is intentionally retained because companies may already use it.
    }
};
