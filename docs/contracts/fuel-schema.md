...
- Business rules:
  - Daily close dips explicitly use opening (morning, before trading) or closing (after trading). New forms default to opening; legacy submissions without timing retain closing semantics.
  - Morning variance compares against inventory before the selected day’s trading. With no opening baseline, the first morning dip establishes it without a gain/loss posting. Closing inventory is morning dip plus that day’s non-opening stock movements minus sales.
  - Evening variance compares against opening inventory plus that day’s stock movements minus sales. Opening and closing readings are stored separately by reading type.
  - Opening inventory includes prior dated stock movements and same-day opening stock, excluding same-day trading and daily-close adjustments. The next day uses the prior close’s remaining inventory.
